...
알겠습니다. 방금 전에 제가 작성해 드린 풍성한 내용의 README를 바로 복사해서 사용하실 수 있도록 단일 `README.md` 파일 형식으로 정리해 드리겠습니다.

아래 내용을 복사하여 `README.md` 파일에 붙여넣기만 하시면 됩니다.
...
2.  [**핵심 목표 및 성과**](https://www.google.com/search?q=%232-%ED%95%B5%EC%8B%AC-%EB%AA%A9%ED%91%9C-%EB%B0%8F-%EC%84%B1%EA%B3%BC)
3.  [**최종 결과 요약**](https://www.google.com/search?q=%233-%EC%B5%9C%EC%A2%85-%EA%B2%B0%EA%B3%BC-%EC%9A%94%EC%95%BD)
4.  [**전체 워크플로우**](https://www.google.com/search?q=%234-%EC%A0%84%EC%B2%B4-%EC%9B%8C%ED%81%AC%ED%94%8C%EB%A1%9C%EC%9A%B0)
5.  [**실험 설계 및 과정**](https://www.google.com/search?q=%235-%EC%8B%A4%ED%97%98-%EC%84%A4%EA%B3%84-%EB%B0%8F-%EA%B3%BC%EC%A0%95)
      - [5.1. 데이터 분석 및 전처리 전략](https://www.google.com/search?q=%2351-%EB%8D%B0%EC%9D%B4%ED%84%B0-%EB%B6%84%EC%84%9D-%EB%B0%8F-%EC%A0%84%EC%B2%98%EB%A6%AC-%EC%A0%84%EB%9E%B5)
      - [5.2. 실험 A: YOLOv5s - 강인한 Augmentation 중심 접근](https://www.google.com/search?q=%2352-%EC%8B%A4%ED%97%98-a-yolov5s---%EA%B0%95%EC%9D%B8%ED%95%9C-augmentation-%EC%A4%91%EC%8B%AC-%EC%A0%91%EA%B7%BC)
      - [5.3. 실험 B: YOLOv8n - 최신 아키텍처 및 고해상도 전략](https://www.google.com/search?q=%2353-%EC%8B%A4%ED%97%98-b-yolov8n---%EC%B5%9C%EC%8B%A0-%EC%95%84%ED%82%A4%ED%85%8D%EC%B2%98-%EB%B0%8F-%EA%B3%A0%ED%95%B4%EC%83%81%EB%8F%84-%EC%A0%84%EB%9E%B5)
6.  [**성능 분석 및 고찰**](https://www.google.com/search?q=%236-%EC%84%B1%EB%8A%A5-%EB%B6%84%EC%84%9D-%EB%B0%8F-%EA%B3%A0%EC%B0%B0)
7.  [**핵심 코드 리뷰**](https://www.google.com/search?q=%237-%ED%95%B5%EC%8B%AC-%EC%BD%94%EB%93%9C-%EB%A6%AC%EB%B7%B0)
      - [7.1. 모델 학습](https://www.google.com/search?q=%2371-%EB%AA%A8%EB%8D%B8-%ED%95%99%EC%8A%B5)
      - [7.2. 추론 및 후처리](https://www.google.com/search?q=%2372-%EC%B6%94%EB%A1%A0-%EB%B0%8F-%ED%9B%84%EC%B2%98%EB%A6%AC)
8.  [**사용한 기술 스택**](https://www.google.com/search?q=%238-%EC%82%AC%EC%9A%A9%ED%95%9C-%EA%B8%B0%EC%88%A0-%EC%8A%A4%ED%83%9D)
9.  [**프로젝트 회고 및 배운 점**](https://www.google.com/search?q=%239-%ED%94%84%EB%A1%9C%EC%A0%9D%ED%8A%B8-%ED%9A%8C%EA%B3%A0-%EB%B0%8F-%EB%B0%B0%EC%9A%B4-%EC%A0%90)
...
| :--- | :--- | :--- | :--- |
| **실험 A** | `YOLOv5s` | **다양한 Augmentation + 이미지 리사이즈 + 밝기 조절** | **🏆 96.333%** |
| **실험 B** | `YOLOv8n` | 고해상도(High-Resolution) 학습 + 최신 아키텍처 | 96.266% |

최종적으로 **실험 A**의 `YOLOv5s` 기반 모델이 미세한 차이로 더 높은 성능을 기록했으며, 이 모델의 예측 결과를 최종 제출물로 선택했습니다.
...
```mermaid
graph TD
    A[🎯 1. 문제 정의 및 목표 설정] --> B[📊 2. 데이터 분석 (EDA)];
    B --> C[✂️ 3. 데이터 전처리 및 증강 전략 수립];
    C --> D[🧪 4. 실험 설계: Model & Hyperparameter];
    D --> E{실험 A: YOLOv5s};
    D --> F{실험 B: YOLOv8n};
    E --> G[🏋️‍♂️ 5. 모델 학습 및 검증];
    F --> G;
    G --> H[📈 6. 성능 평가 및 분석];
    H --> I[🚀 7. 추론 및 후처리];
    I --> J[📤 8. 최종 결과 제출];
...
### 5.2. 실험 A: YOLOv5s - 강인한 Augmentation 중심 접근

  - **모델:** `YOLOv5s`. v5 시리즈는 안정성과 속도, 정확도 간의 균형이 뛰어나 베이스라인 및 성능 최적화에 적합하다고 판단했습니다.
  - **핵심 전략:**
    1.  **강력한 데이터 증강:** 위에서 설계한 `Albumentations` 파이프라인을 적용했습니다. 특히 `RandomBrightnessContrast`의 강도를 높게 설정했습니다.
    2.  **이미지 리사이즈:** 모든 이미지를 `640x640`으로 통일하여 학습 안정성을 확보했습니다.
  - **주요 하이퍼파라미터:**
      - `imgsz`: 640
      - `batch`: 16
      - `epochs`: 100
...
### 5.3. 실험 B: YOLOv8n - 최신 아키텍처 및 고해상도 전략

  - **모델:** `YOLOv8n`. Anchor-free 방식과 개선된 백본을 특징으로 하는 최신 아키텍처의 성능을 확인하고자 했습니다.
  - **핵심 전략:**
...
```python
# YOLOv5s 모델 로드 (실험 A 기준)
from ultralytics import YOLO

# 사전 학습된 YOLOv5s 모델을 로드합니다.
...
## 8\. 사용한 기술 스택
  ...
  - **핵심 라이브러리:**
      - `PyTorch`: 딥러닝 프레임워크
      - `ultralytics`: YOLOv5, YOLOv8 모델 구현 및 학습/추론
